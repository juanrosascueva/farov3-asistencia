...
export default function Dashboard({ teens, attendanceMap, onOpenProfile, onNavigate }: DashboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {isSupervisor && <section className="rounded-card border border-teal-100 bg-teal-50/70 p-4 sm:p-5"><SectionHeading title="Sin responsable" helper="Adolescentes que no tienen líder individual ni líder de grupo." /><div className="mt-3 grid gap-2 lg:grid-cols-2">{portfolio?.unassigned?.length ? portfolio.unassigned.map((item: any) => <div key={item.teenId} className="flex flex-wrap items-center gap-2 rounded-lg border border-teal-100 bg-card px-3 py-2.5"><button onClick={() => onOpenProfile(String(item.teenId))} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-bold text-ink">{item.teenName}</p><p className="mt-0.5 truncate text-xs text-teal-700">{item.reason}</p></button><select aria-label={`Asignar líder a ${item.teenName}`} defaultValue="" onChange={(event) => assignFromDashboard(String(item.teenId), event.target.value)} className="rounded-lg border border-teal-100 bg-card px-2 py-1.5 text-xs font-semibold text-ink"><option value="" disabled>Asignar...</option>{assignableUsers.map((user: any) => <option key={user._id} value={user._id}>{user.name}</option>)}</select></div>) : <p className="text-sm text-teal-700">Todos los adolescentes de este ámbito tienen un responsable.</p>}</div></section>}
      {isSupervisor && summary?.metrics?.teamCoverage?.length > 0 && <section className="rounded-card border border-ink/10 bg-card p-4 sm:p-5"><SectionHeading title="Cobertura del equipo" helper="Cartera actual, tareas abiertas y carga declarada por líder." /><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{summary.metrics.teamCoverage.map((item: any) => <div key={item.userId} className="rounded-lg border border-ink/10 p-3"><p className="truncate text-sm font-bold">{item.name}</p><p className="mt-1 text-xs text-ink/55">{item.assigned}/{item.capacity} adolescentes · {item.openTasks} tareas</p>{item.overdue > 0 && <div className="mt-1 flex items-center justify-between gap-2"><p className="text-xs font-semibold text-coral-700">{item.overdue} vencida(s)</p>{item.phone && <a href={`https://wa.me/${item.phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${item.name}, tienes ${item.overdue} tarea(s) pastoral(es) vencida(s). Por favor revísalas en Teens.`)}`} target="_blank" rel="noreferrer" className="text-xs font-semibold text-teal-700 underline">Recordar</a>}</div>}</div>)}</div></section>}
...
function PriorityList({ items, onOpenProfile, onStart, empty = "No hay prioridades abiertas." }: any) { return items.length ? <div className="mt-4 grid gap-2 lg:grid-cols-3">{items.map((item: any) => <div key={item.taskId} className="rounded-lg border border-teal-100 bg-teal-50 px-3 py-3"><button onClick={() => onOpenProfile(String(item.teenId))} className="w-full text-left"><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-semibold text-ink">{item.teenName}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${item.priority === "critical" ? "bg-coral-50 text-coral-700" : "bg-card text-teal-700"}`}>{priorityLabel[item.priority] || "Media"}</span></div><p className="mt-1 line-clamp-2 text-xs text-teal-700">{item.title}: {item.detail}</p><p className="mt-2 text-[11px] text-ink/55">{item.assignedName}{item.dueDate ? ` · Vence ${item.dueDate}` : ""}</p></button><button onClick={() => onStart(item)} className="mt-3 game-pill game-pill--outline !min-h-0 !px-3 !py-1 text-xs">En proceso</button></div>)}</div> : <p className="mt-4 text-sm text-ink/45">{empty}</p>; }
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) { return <div className="game-stat"><p className="text-xs font-semibold text-ink/45">{label}</p><p className="mt-1 text-2xl font-bold tabular-nums text-teal-700">{value}</p><p className="mt-1 text-xs text-ink/40">{detail}</p></div>; }
