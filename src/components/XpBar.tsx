    ...
    <div className="bg-card rounded-card shadow-soft p-5">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className="w-7 h-7 rounded-lg bg-primary-600 flex items-center justify-center text-white text-xs font-bold">
            {level.level}
          </span>
          <div>
            <p className="text-sm font-semibold leading-tight">Nivel {level.level}</p>
            <p className="text-xs text-ink/45 leading-tight">{level.name}</p>
          </div>
        </div>
      ...
      {level.xp < level.nextXp && (
        <p className="text-xs text-ink/40 mt-1.5 text-right">
