...
export default function PublicTeenRegistration({ publicToken, shortCode }: { publicToken: string; shortCode: string }) {
  ...
  return <main className="min-h-dvh bg-paper px-4 py-6 sm:py-10 text-ink"><div className="mx-auto w-full max-w-2xl"><div className="mb-5"><p className="text-xs font-bold uppercase text-primary-700">Ministerio de adolescentes</p><h1 className="font-display text-balance text-2xl sm:text-3xl font-bold mt-1">Registro básico de adolescentes</h1><p className="text-sm text-pretty text-ink/55 mt-2">Puede ser llenado por el adolescente, su padre, madre o apoderado.</p>{scopeLabel && <p className="text-sm font-semibold text-ink/65 mt-2">Asignación: {scopeLabel}</p>}</div>
    <form onSubmit={handleSubmit} className="bg-card rounded-card shadow-soft border border-ink/5 p-4 sm:p-6 space-y-4">{error && <div className="rounded-xl border border-danger-100 bg-danger-50 px-3 py-2 text-sm font-medium text-danger-700">{error}</div>}
