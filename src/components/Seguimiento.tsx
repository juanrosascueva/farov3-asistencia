...
export default function Seguimiento({ onOpenProfile }: { onOpenProfile: (id: string) => void }) {
  ...
  return <div className="space-y-5">
    <header><p className="text-xs font-semibold uppercase text-teal-700">Acompañamiento pastoral</p><h1 className="mt-0.5 font-display text-2xl font-bold">Seguimiento</h1><p className="mt-1 text-sm text-ink/50">{scopeLabel}</p></header>
    <div className="flex gap-2 overflow-x-auto pb-1">{([ ["all", "Todo"], ["mine", "Para mí"], ["overdue", "Vencidas"], ["high", "Alta prioridad"], ["signals", "Señales"] ] as [Filter, string][]).map(([id, label]) => <button key={id} onClick={() => setFilter(id)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-bold ${filter === id ? "bg-ink text-white" : "border border-ink/10 bg-card text-ink/55"}`}>{label}</button>)}</div>
    <div className="space-y-2">{items.length ? items.map((item: any) => <div key={item.id} className="rounded-card border border-ink/10 bg-card p-4 shadow-soft"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><button onClick={() => onOpenProfile(String(item.teenId))} className="min-w-0 text-left"><p className="text-sm font-bold">{item.teenName}</p><p className="mt-1 text-sm font-semibold text-ink/75">{item.title}</p><p className="mt-1 text-xs text-ink/50">{item.detail}</p><p className="mt-2 text-[11px] text-ink/40">{item.assignedName || "Sin responsable"}{item.dueDate ? ` · Vence ${item.dueDate}` : ""}</p></button><div className="flex shrink-0 flex-wrap gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.priority === "critical" ? "bg-coral-50 text-coral-700" : "bg-teal-50 text-teal-700"}`}>{priorityLabel[item.priority] || "Media"}</span>{item.kind === "task" ? <><button onClick={() => updateStatus({ token: token!, taskId: item.taskId, status: "in_progress" })} className="rounded-lg border border-ink/10 px-2 py-1 text-xs font-semibold">En proceso</button><button onClick={() => updateStatus({ token: token!, taskId: item.taskId, status: "done" })} className="rounded-lg bg-teal-600 px-2 py-1 text-xs font-semibold text-white">Realizada</button><select value={item.assignedToUserId || ""} onChange={(event) => event.target.value && reassign({ token: token!, taskId: item.taskId, assignedToUserId: event.target.value as any })} className="rounded-lg border border-ink/10 bg-card px-2 py-1 text-xs font-semibold"><option value="">Asignar...</option>{assignableUsers.map((person: any) => <option key={person._id} value={person._id}>{person.name}</option>)}</select></> : <button onClick={() => createFromSignal(item)} className="rounded-lg bg-teal-600 px-3 py-2 text-xs font-bold text-white">Crear tarea</button>}</div></div></div>) : <div className="rounded-card border border-ink/10 bg-card p-8 text-center text-sm text-ink/45">No hay seguimientos en esta vista.</div>}</div>
  </div>;
}
