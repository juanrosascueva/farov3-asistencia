...
export const getLevel2Summary = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Motivos son texto libre; no pueden usarse como claves de objetos Convex porque pueden incluir tildes.
    const reasonCounts = new Map<string, number>(); attendance.filter(a => a.status === "excused" && a.excuseReason).forEach(a => reasonCounts.set(a.excuseReason!, (reasonCounts.get(a.excuseReason!) || 0) + 1));
    ...
    return { attendanceByType: Object.entries(byType).map(([type, value]) => ({ type, ...value, rate: value.total ? Math.round(value.present / value.total * 100) : 0 })), milestones: { total: milestones.length, byType: milestones.reduce((out: Record<string, number>, item) => { out[item.type] = (out[item.type] || 0) + 1; return out; }, {}) }, absenceReasons: Array.from(reasonCounts, ([reason, count]) => ({ reason, count })), taskCompletion: { open: tasks.filter(t => ["pending", "in_progress", "rescheduled", "escalated"].includes(t.status)).length, done: tasks.filter(t => t.status === "done").length }, consentPending: teens.filter(t => !t.consentimientoDatos || !t.consentimientoFoto).length };
  },
});
